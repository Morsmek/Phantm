    ...
    fun getOrGenerateDatabaseKey(context: Context): String {
        ...
        if (encryptedKeyBase64 != null && ivBase64 != null) {
            try {
                ...
                // Keystore failure or decryption failure. 
                // Do not fallback, fail hard to prevent data loss or unencrypted state.
                throw IllegalStateException("Failed to decrypt database key from KeyStore", e)
            }
        }
        ...
        try {
            ...
            // Do not swallow KeyStore errors. Crash the app rather than entering an unrecoverable or unencrypted state.
            throw IllegalStateException("Failed to encrypt and store new database key", e)
        }
        return newRawKey
    }
